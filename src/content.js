...
Please provide suggestions in the following JSON format:
{
  ...
  async handleInput(event) {
    ...
    // Skip processing for delete/backspace
    ...
    // Check URL filtering
    if (this.settings?.urlFilterEnabled && this.settings?.allowedUrls?.length > 0) {
      ...
      if (!isAllowed) {
        console.log('URL not in allowed list, skipping check');
        return;
      }
    }
